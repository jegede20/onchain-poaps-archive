...
export default function VerifyPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      <Link href="/" className="text-sm text-muted hover:text-ink">← Archive</Link>
      <h1 className="mt-4 text-3xl font-semibold tracking-tight">Verify attendance</h1>
      <p className="mt-2 text-muted">Did a wallet really earn a POAP? Check directly against the chain — links to the raw receipt.</p>

      <div className="mt-8 archive-card p-6 space-y-5">
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label className="text-sm font-medium">POAP</label>
            <select value={poapId} onChange={e=>setPoapId(e.target.value)} className="mt-1 w-full rounded-xl border border-line bg-white px-3 py-3 text-sm">
            ...
            <div className="text-xs text-muted mt-1">Total {totalNum} onchain • newest first in Explore</div>
          </div>
          <div>
            <label className="text-sm font-medium">Wallet address</label>
            <input value={wallet} onChange={e=>setWallet(e.target.value)} placeholder="0x..." className="mt-1 w-full rounded-xl border border-line bg-white px-3 py-3 mono-num text-sm" />
          </div>
        </div>
        <button onClick={doCheck} disabled={isFetching} className="ink-button w-full">{isFetching ? 'Checking…' : 'Check ✓'}</button>
            ...
            { !/^0x[a-fA-F0-9]{40}$/.test(wallet) ? <div className="text-sm text-warn">Enter a valid address to verify.</div> :
              hasClaimed ? (
                <div>
                  ...
                  <div className="mt-3 mono-num text-xs leading-6">
      ...
      <div className="mt-6 archive-inset p-5">
        <h3 className="font-semibold text-sm">How verification works</h3>
